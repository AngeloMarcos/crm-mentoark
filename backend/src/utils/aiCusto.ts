...
export const PRECO_POR_1M_TOKENS: { prefixo: string; input: number; output: number }[] = [
  ...
  // economizar ao máximo token"): gpt-5-nano/gpt-5-mini adicionados como opção nas telas de
  // configuração (Integracoes.tsx e demais, ver comentário lá) — preço confirmado em 2+ fontes
  // independentes de pricing (referência: setembro/2026). Ordem importa: `startsWith` casa o
  // PRIMEIRO prefixo que bater, então entradas mais específicas (`gpt-5-nano`) vêm antes de
  // prefixos mais genéricos que também bateriam (`gpt-5`, se um dia for adicionado).
  { prefixo: 'gpt-5-nano',             input: 0.05,  output: 0.40  },
  { prefixo: 'gpt-5-mini',             input: 0.25,  output: 2.00  },
  { prefixo: 'gpt-4.1-nano',           input: 0.10,  output: 0.40  },
  { prefixo: 'gpt-4.1-mini',           input: 0.40,  output: 1.60  },
  { prefixo: 'gpt-4.1',                input: 2.00,  output: 8.00  },
  { prefixo: 'gpt-4o-mini',            input: 0.15,  output: 0.60  },
  { prefixo: 'gpt-4o',                 input: 2.50,  output: 10.00 },
  { prefixo: 'gpt-3.5-turbo',          input: 0.50,  output: 1.50  },
