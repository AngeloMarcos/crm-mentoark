  ...
  return (
    <CRMLayout>
      <div className="flex flex-col gap-4 h-full">
        {/* Header */}
        <div className="rounded-xl border border-border bg-card/60 backdrop-blur-sm p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Gerencie as tarefas e fluxos da sua equipe
            </p>
          </div>
          <Button
            onClick={() => {
        ...
        {/* Filtros */}
        <div className="rounded-xl border border-border bg-card/40 backdrop-blur-sm px-4 py-3 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold text-muted-foreground uppercase">Membros:</span>
            <div className="flex -space-x-2">
              <button
                onClick={() => setFiltroMembro(null)}
                className={cn(
                  "relative z-10 w-8 h-8 rounded-full border-2 border-background bg-muted text-[10px] font-bold text-muted-foreground flex items-center justify-center hover:bg-muted/80 transition-colors",
                  !filtroMembro && "ring-2 ring-primary ring-offset-2 ring-offset-background"
                )}
              >
                ALL
              </button>
              {membros.map((m) => (
                <button
                  ...
                  className={cn(
                    "relative w-8 h-8 rounded-full border-2 border-background overflow-hidden hover:scale-105 transition-transform",
                    filtroMembro === m.membro_id && "ring-2 ring-primary ring-offset-2 ring-offset-background z-20"
                  )}
                  title={m.nome}
                >
          ...
          <div className="flex items-center gap-1.5">
            {["Todas", "Urgente", "Alta", "Média", "Baixa"].map((p) => {
              const slug = p.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
              const active = (p === "Todas" && !filtroPrioridade) || slug === filtroPrioridade;
              return (
                <Badge
                  key={p}
        ...
        {/* Board */}
        <div className="flex-1 rounded-xl border border-border bg-muted/10 backdrop-blur-sm overflow-hidden relative min-h-[500px]">
          {loading && colunas.length === 0 ? (
            <div className="flex gap-4 p-6">
              {[1, 2, 3].map((i) => (
                <div key={i} className="w-80 space-y-3 shrink-0">
            ...
            <ScrollArea className="h-full w-full">
              <div className="flex p-4 gap-4 min-h-[500px]">
                {colunas.map((coluna) => (
                  <KanbanColuna
                    key={coluna.id}
                    coluna={coluna}
          ...
          {/* FAB */}
          <Button
            className="absolute bottom-6 right-6 w-14 h-14 rounded-full shadow-xl ring-4 ring-background"
            onClick={() => {
              setTarefaEditando(undefined);
              setColunaInicial(undefined);
              setModalAberto(true);
