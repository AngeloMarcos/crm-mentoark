# Prompt Lovable B — Sidebar: Organização por Categorias

## Objetivo
Reorganizar a sidebar em **6 grupos com labels de categoria**, mantendo o visual atual (dark, gradiente, ícones coloridos). Os grupos têm labels visíveis quando expandido e colapsam elegantemente quando a sidebar está recolhida.

---

## Contexto
Este prompt substitui completamente `src/components/AppSidebar.tsx`. Aplicar **somente após o Prompt A** (ícones corrigidos). O visual e comportamento do item ativo (gradiente, barra lateral, hover) devem ser mantidos idênticos.

---

## Arquivo: `src/components/AppSidebar.tsx` — substituição completa
...
| Antes | Depois |
|-------|--------|
| Lista plana sem separação | 6 grupos com label de categoria |
| Sem hierarquia visual | Labels em UPPERCASE, tom sutil |
| Campanhas e Marketing Digital misturados com tudo | Agrupados em "Comunicação" |
| IA distribuída aleatoriamente | Agrupada em "Automação & IA" |
| Sem forma de colapsar categorias | Clique no label para colapsar/expandir o grupo |
| Sidebar colapsada sem separação | Divisores finos entre grupos quando recolhida |

## Comportamento dos grupos
- **Expandido por padrão** — todos os grupos abertos ao carregar
- **Colapso independente** — cada grupo pode ser colapsado individualmente clicando no label
- **Grupo do item ativo sempre aberto** — se você está em Marketing Digital, o grupo "Comunicação" nunca colapsa automaticamente
- **Sidebar recolhida** — labels somem, ícones permanecem, divisores finos separam os grupos

---

## Não alterar
Nenhum outro arquivo. Apenas `src/components/AppSidebar.tsx`.
