    ...
    <div className="px-2">
      <button
        ...
        className={`flex items-center w-full gap-2.5 px-2.5 py-2 rounded-lg transition-all duration-200 ${
          hasActive
            ? "gradient-brand-subtle shadow-[inset_0_1px_0_hsl(var(--primary)/0.08),0_0_12px_hsl(var(--primary)/0.10),inset_0_0_0_1px_hsl(var(--primary)/0.14)]"
            : "hover:bg-sidebar-accent hover:shadow-[inset_0_1px_1px_hsl(var(--primary)/0.03)]"
        }`}
      >
        <Icon className={`h-[18px] w-[18px] shrink-0 transition-all duration-300 ${hasActive ? (subgroup.color || "text-primary") + " drop-shadow-[0_0_4px_hsl(var(--primary)/0.25)] scale-105" : "text-muted-foreground"}`} />
              ...
              return (
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton asChild>
                    ...
                    >
                      {active && (
                        <span className="absolute -left-3 top-1/2 -translate-y-1/2 h-5 w-[2px] rounded-r gradient-brand shadow-[0_0_10px_hsl(var(--primary)/0.7)]" />
                      )}
                      <item.icon
