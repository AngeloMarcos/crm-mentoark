...
**Estado atual:** recebimento de mensagens **continua quebrado** (P2010); envio funcionando normalmente — ambas tentativas de correção (upgrade 2.4.x e migração Postgres) foram revertidas no mesmo dia após confirmarem problemas em produção. `FIX PENDENTE`. Hipótese do dialeto Postgres tecnicamente confirmada (migrations rodam limpo), mas a migração de banco por si só não é suficiente — falta descobrir como migrar/recriar os registros da tabela `Instance` (dados que hoje só existem no MySQL) para o Postgres novo sem perder o vínculo com os arquivos de sessão já pareados, ou usar o endpoint de criação de instância pra tentar re-vincular pelos IDs de pasta existentes (`8885c510-4faa-4511-ab07-716ccdebd1ed` = `crm_435ee4720fc3`). **Recomendação:** próxima tentativa não deve ser feita direto em produção — vale primeiro testar essa recuperação (import de dados da tabela `Instance` do MySQL pro Postgres, ou recriação via API) num container Evolution temporário/isolado, usando uma cópia dos arquivos de `/evolution/instances/`, antes de repetir a troca no serviço compartilhado real.
